...
| `src/ai_psi/domain/experiences.py` | 129 | 129 | **100.0%** | 0 | 0 | 22 | 13 |
...
- **core/ReplaceComparisonOperator_Gt_NotEq** @ 第 649 行（覆盖 1 条）
  ```diff
  -        and evaluation.rank > ExperienceEvaluation.SUSPECTED.rank
  +        and evaluation.rank != ExperienceEvaluation.SUSPECTED.rank
  ```
  **等价理由**：同上：rank 的四个取值互异且都在小整数缓存内，`!=` 与 `>` 同答案
- **core/ReplaceComparisonOperator_Gt_IsNot** @ 第 649 行（覆盖 1 条）
  ```diff
  -        and evaluation.rank > ExperienceEvaluation.SUSPECTED.rank
...
- **core/ReplaceComparisonOperator_Is_GtE** @ 第 640 行（覆盖 1 条）
  ```diff
...
- **core/NumberReplacer** @ 第 289 行（覆盖 1 条）
  ```diff
  -        min_length=1,
  +        min_length= 2,
  ```
  **等价理由**：同上，方向反过来：派生值最短也有 5 个字符，抬到 2 拒不掉任何东西
- **core/ReplaceTrueWithFalse** @ 第 522 行（覆盖 1 条）
  ```diff
  -@dataclass(frozen=True, slots=True)
  +@dataclass(frozen=True, slots=False)
  ```
...
- **core/NumberReplacer** @ 第 297 行（覆盖 1 条）
  ```diff
  -        min_length=1,
  +        min_length= 2,
  ```
  **等价理由**：同上，方向反过来：把下界抬到 2 也不会拒掉任何东西——派生值同样是 42 字符起步。两半都要登记，因为「没有输入能走到这里」对**两侧**都成立
- **core/ReplaceComparisonOperator_Is_Eq** @ 第 648 行（覆盖 1 条）
  ```diff
...
- **core/ReplaceComparisonOperator_IsNot_Lt** @ 第 632 行（覆盖 1 条）
  ```diff
...
- **core/ReplaceComparisonOperator_Gt_GtE** @ 第 591 行（覆盖 1 条）
  ```diff
...
- **core/ReplaceComparisonOperator_Is_Eq** @ 第 640 行（覆盖 1 条）
  ```diff
  -    if evaluation is ExperienceEvaluation.UNASSESSED:
  +    if evaluation == ExperienceEvaluation.UNASSESSED:
  ```
...
- **core/ReplaceComparisonOperator_IsNot_NotEq** @ 第 632 行（覆盖 1 条）
  ```diff
  -        if evaluation is not ExperienceEvaluation.UNASSESSED:
  +        if evaluation != ExperienceEvaluation.UNASSESSED:
  ```
