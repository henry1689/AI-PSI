...
class RoundRepository(Protocol):
    ...
    async def list_all(self, *, limit: int | None = None) -> list[CognitiveRound]:
        """列出回合（阶段 6.5 §四：离线评测的读取入口）。

        🔴 **顺序必须确定**：``created_at`` 升序，同一时刻按 id 升序。

        用升序而不是降序，是因为这个方法的调用方是**评测**：
        它要把历史切成"基线"与"候选"两段，而"最早的 N 条"
        在升序下是一个稳定的窗口；降序则要先把全部读出来再翻面。

        ⚠️ **只有在这一层定义顺序，"顺序确定"才是一句可验证的话。**
        让每个调用方自己排序，等于把一条契约拆成 N 份，
        而其中任何一份写错都不会有人发现。
