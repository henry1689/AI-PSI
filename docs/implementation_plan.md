...
> **本文件是阶段进度的权威来源之一，不是唯一来源。**
> 每个阶段结束时必须更新此表。
> 相关：ADR-0012（范围边界与目录偏差）
>
> ⚠️ **2026-09-19 更正**：本节此前自称"**唯一**真相来源"，而它当时是**错的**
> ——下表把阶段 6 写成"未开始"，与同文件 §3 的小节标题 ✅、
> 与 `README.md` 的进度表、与 `docs/assurance/` 的两份阶段报告**互相矛盾**。
> 一份自称唯一的文档一旦过期，比没有它更有害：读者会据此**停止核对别处**。
>
> 现在的事实分工是：
>
> | 想知道什么 | 看哪里 |
> |---|---|
> | 阶段**走到哪一步**（进度） | 本表 + `README.md` 的进度表（两者必须一致） |
> | 某项能力**证据到什么等级** | `docs/assurance/capability_evidence_matrix.md` |
> | 已知问题与残余风险 | `docs/risks.md` |
> | 为什么这么设计 | `docs/adr/` |
>
> **四者不一致时以能力矩阵为准**（它的填写规则里写明了"文档里写了不是证据"），
> 并把差异记为待办。
...
| **6** | 反馈、经验与改进提案 | ✅ 完成 | 2026-09-18 |
| **6.5** | 阶段 6 的完成定义修订（经验身份/评价/独立性分组、输入契约、门禁权威边界、变异测试） | ✅ 完成 | 2026-09-18 |
| **6.6** | 真实错误归因闭环（用户指定专项） | ✅ 完成 | 2026-09-19 |
| **7‑专项** | **R72 并发唯一性加固**（进入阶段 7 后完成的**专项**，见下） | ✅ 完成 | 2026-09-19 |
| **7** | **评测与回放（任务书正式路线图）** | ⬜ **未开始** | — |
| **8** | 完整验收与交付 | ⬜ 未开始 | — |

> 🔴 **"7‑专项"与"7"是两件事，不要合读。**
>
> **R72 是阶段 7 开始后完成的专项加固**——它修的是阶段 6.6 独立评审发现的
> 并发缺陷（两个并发的学习运行会写出内容相同的 DRAFT 提案），
> 交付的是一条数据库级唯一性约束及其证据。
> 它是**用户指定的范围**，**不是**任务书 §18 定义的阶段 7 交付物。
>
> **正式阶段 7 路线图一项都还没开始**：
> 50 个 Golden Cases、Eval Runner、指标、Markdown/JSON 报告、
> 版本比较——全部为**未开始**。
> 因此**不要**从"7‑专项 ✅"推断出"阶段 7 已完成"，
> 也不要从"阶段 7 ⬜"推断出"R72 没做"。
>
> 同理，`README.md` 的进度表把 R72 列在阶段 6 之后、阶段 7 之前，
> 表述与本表一致。
...
⚠️ **Anthropic Provider：经 ADR-0016 §1 批准的「延后」，不是未登记的缺陷。**

任务书 §8.1 / §18 阶段 4 要求实现 Anthropic Provider；ADR-0016 §1
（状态：**已接受**）正式决定本阶段**不写**它，理由是"写一个既没有测试、
又无法实际跑通的 Provider，产出的是看起来有能力的空壳"（ADR-0012 明令避免）。
配置它会在装配时**明确报错**而不是静默回落。

**定性**：这是一次**经 ADR 批准的延后**——比"已知设计偏差"更明确，
但**不等于需求已被永久替代**：任务书的那条要求没有被删除，
只是被正式推迟，且 ADR 写明了后续接入路径
（`providers/` 下一个新文件 + 注册表一行）。
**不要**把它记为缺陷，也**不要**据此认为 Anthropic 已被放弃。
...
**独立评审与修正（ADR-0019）**：阶段 6 交付后补做了一次三视角的
独立评审（互不通气、只读），抓到 **11 处确认缺陷**，其中最重要的一条是
**验收条件本身在跑起来的系统里不成立**——`learning/` 六个模块在 `src/` 里
零调用者，运行时产出的经验 `error_type` 被硬编码成 `None`，
模式发现永远不会说话。全部缺陷已修，并新增
`tests/scenarios/test_learning_chain.py` **只用真实回合产出的事件**
证明"三次同类错误 → 提案"这条链路真的通。

评审同时推翻了 ADR-0018 的三处**表述**（文档说过头、代码没错），
以及三条"测试在假装验证"的断言。详见 ADR-0019。
