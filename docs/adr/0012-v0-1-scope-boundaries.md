...
以下三项在写代码/测试时才暴露，同样登记于此。

| # | 缺口 | 决策 | 落地位置 |
|---|---|---|---|
| **G1** | 任务书 §5.2 的事件清单实际是 **31 条**，其中 `cognitive_round.cancelled` **缺失**——但 §6.1 的状态机有 `CANCELLED` 状态。不补的话**取消回合不留任何审计轨迹**，违反"重要对象保留变更事件" | 补齐 `cognitive_round.cancelled`，使总数成为 32。测试断言**每个终态都有对应事件类型**，防止未来再漏 | `domain/enums.py` `EventType` |
| **G2** | §4 目录结构列了 `domain/situations.py`，但 §5 的领域模型章节**没有 Situation 的定义**——只在 `Experience.situation_signature` 里隐含引用了一个字符串。没有情境概念，"同类错误至少出现 3 次"（§11.3 提案门槛）**无法判定**，"同类"无从比较 | 给出**最小**定义：可比较的签名字符串 + 少量显式特征。刻意不做完整世界建模 | `domain/situations.py` |
| **G3** | §9.10 要求 Judgment"必须生成最强反证"，但 §9.8 又**禁止机械地"双方都有道理"**。两者在简单事实上冲突：对"水在标准大气压下的沸点"，强行制造反方观点恰恰是被禁止的行为 | **允许 `strongest_counterarguments` 为空**；不变量 3 改由另一条可测规则承载——`EpistemicAction.ANSWER`（无保留结论）要求 `unresolved_unknowns` 为空 | `domain/judgments.py` |

> **关于 G3 的取舍**：把"必须有反证"写成硬校验，会逼迫模型为简单事实**编造反方观点**，
> 直接违反 §9.8。而"存在未解决未知时不得无保留确定"这条规则，
> 既守住了不变量 3 的实质（不夸大确定性），又不会制造虚假平衡。
> 这也说明任务书里的两条要求需要**分开**实现，而不是都塞进一个字段的必填校验。
